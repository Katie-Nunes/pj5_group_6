...
def create_garage_trips(timetable, distance_matrix, index,  locations, charging_dict, bus_id=None, current_energy=None):
    ...
    if bus_id is not None: # we assume that if you give a bus id, then you want to go to the garage
        ...
        record, cur_energy = create_material_record(row_values["start"], 'ehvgar', row_values["line"], row_values["departure_time"], distance_matrix, bus_id, current_energy)
        ...
        if not locations.get("ehvgar", []): # Garage doesn't have any buses
            ...
            record, _ = create_material_record('ehvgar', row_values["start"], row_values["line"], pre_trip_depart, distance_matrix, new_bus_id, 255)  # new bus always start at max of SoH
            return record, None
        ...
        if not q:
            ...
            record, _ = create_material_record('ehvgar', row_values["start"], row_values["line"], row_values["departure_time"], distance_matrix, new_bus_id, 255)  # new bus always start at max of SoH
        else:
            ...
            record, _ = create_material_record('ehvgar', row_values["start"], row_values["line"], row_values["departure_time"], distance_matrix, best_bus, bus_pre_depart_energy)
...
def main_timetable_iteration(timetable, distance_matrix, locations, charging_dict):
    ...
    for index, row in timetable.iterrows():
        ...
        if not busses_at_current_location: # FOR SOME REASON ONLY EVER RUNS(CHARGES) TWICE??
            ...
            print(f"charging_trip: {charging_trip}")  # Debug: See what you're getting
            print(f"pre_trip: {pre_trip}")  # Debug
            ...
            if current_energy > 51: #20% (at this point we just hope it doesn't dip below 10%)
                ...
                send_to_garage, _ = create_garage_trips(timetable, distance_matrix, index, locations, charging_dict, bus_id, current_energy) # Assumes energy doesn't run out while idling
