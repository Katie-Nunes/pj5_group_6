...
def check_locations(df, timetable, distancematrix, discard):
    ...
    if not (df_locations == timetable_locations == distancematrix_locations):
        return ValueError(f"Location mismatch: Dataframes have inconsistent location sets")
    return True
...
def _coerce(series, ref_date):
    ...
    try:
        ...
        errors.append(f"Failed with format '%H:%M:%S'")
    ...
    try:
        ...
        errors.append(f"Failed with format '%H:%M'")

    # Try general datetime parsing
    try:
        t = pd.to_datetime(series.astype(str)).dt.time
        return pd.to_datetime([datetime.combine(ref_date, x) for x in t])
    except Exception as e:
        errors.append(f"Failed with general datetime parsing")

    # If all methods fail, raise an error with all attempts
    error_msg = "Could not parse time data. Attempted formats:\n" + "\n".join(errors)
...
### HOW TO HANDLE THIS WITH @streamlit_error_handler
def check_datetime_sequence(df):
    ...
    if not (df['end time'] > df['start time']).all():
        ...
        #Raise Value error but continue if user allows
...
@streamlit_error_handler ## HOW TO HANDLE?
...
@streamlit_error_handler
def check_for_inaccuracies(df, timetable, distancematrix, expected_columns=['start location', 'end location', 'start time', 'end time', 'activity', 'line', 'energy consumption', 'bus'], expected_dtypes={'start location': dtype('O'), 'end location': dtype('O'), 'start time': dtype('datetime64[ns]'), 'end time': dtype('datetime64[ns]'), 'activity': dtype('O'), 'line': dtype('float64'), 'energy consumption': dtype('float64'), 'bus': dtype('int64')}, too_long_for_idle_in_minutes=120, idle_cost_ph=5, charge_speed_assumed=7.5, low_charge_rate=0.9, high_charge_rate=1.1, low_energy_use=0.7, high_energy_use=2.5, low_idle_cost=0.9, high_idle_cost=1.1, discard='ehvgar', ref_date=None):
